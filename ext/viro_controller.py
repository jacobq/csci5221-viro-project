...
class ViroSwitch(object):
  ...
  def createOPENFLOW_Message(self, openflow_port, mac, packet, event_port = None):
       ...
       msg.data = e.pack()
       msg.actions.append(of.ofp_action_output(port = openflow_port)) # send the message to the same port as the openflow port
  ...
  ###############################################
       #  RunARound function starts here
  ###############################################
  ...
  ###############################################
         #    startRound function starts here
  ###############################################
         ...
         for i in range(1,L+1):
            if i in myViro.routingTable:
                for j in myViro.routingTable[i]:
                   print 'Bucket::', i, 'Nexthop:',bin2str(j[0],L),'Port:',j[2], 'Gateway:',bin2str(j[1],L), 'Prefix:',j[3]
            else:
