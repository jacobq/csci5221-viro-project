...
def getdpidLength(dpid):
    dpidbytes = dpid.split("-")
    return len(dpidbytes) * 8
...
def createDISCOVER_ECHO_REQ(myvid, dpid):
    ...
    dstvid_array = getMacArray(dpid)
    sdpid = struct.pack("!BBBBBB", dstvid_array[0], dstvid_array[1], dstvid_array[2], dstvid_array[3], dstvid_array[4],
                        dstvid_array[5])
    ...
    return fwd + res + packet + svid + sdpid


def createDISCOVER_ECHO_REPLY(myvid, dpid):
    # First prepare header
...
def createVIRO_DATA(src_vid, dst_vid, fwd_vid, ttl, payload):
    packet = struct.pack("!HHBBH", HTYPE, PTYPE, HLEN, PLEN, VIRO_DATA_OP)
    svid = struct.pack("!I", int(src_vid, 2))
    dvid = struct.pack("!I", int(dst_vid, 2))
    p = struct.pack("!I", payload)
    fwd = struct.pack('!I', int(dst_vid, 2))
    res = struct.pack('!HH', 0x0000, VIRO_CONTROL)

    return fwd + res + packet + svid + dvid + p
...
def createRDV_PUBLISH(bucket, myvid, dst):
    ...
    return (fwd + res + packet + svid + dvid + z)
...
def createRDV_REPLY(gw, bucket_dist, myvid, dst):
    ...
    return (fwd + res + packet + svid + dvid + buck_dist + z)
...
def createRDV_WITHDRAW(failedNode, myvid, dst):
    print 'create_RDV', myvid, dst, failedNode
    # First prepare header
...
def createGW_WITHDRAW(failedGW, myvid, dst):
    #print 'create_GW Withdraw', myvid, dst, failedGW
    # First prepare header
    ...
    dvid = struct.pack("!I", int(dst, 2))
    # Destination Subtree-k
    z = struct.pack("!I", int(failedGW, 2))
...
def getopcode(packet):
    [opcode] = struct.unpack("!H", packet[14:16])
...
def printPacket(packet, L):
    [opcode] = struct.unpack("!H", packet[14:16])  # 6:8
    [svid] = struct.unpack("!I", packet[16:20])
    [dvid] = struct.unpack("!I", packet[20:24])
    ...
    if opcode not in OPERATION_NAMES:
        ...
        return
    print 'Type: ', OPERATION_NAMES[opcode],\
        'Source: ', bin2str(svid, L),\
        'Destination: ', bin2str(dvid, L),\
        'Payload: ', bin2str(payload, L)


# print DiscoveryPacket
def printDiscoverPacket(packet, L, length):
    [opcode] = struct.unpack("!H", packet[14:16])  # 3:5
    ...
    if opcode not in OPERATION_NAMES:
        ...
        #return 
    #print 'Type: ',OPERATION_NAMES[opcode], 'VID: ', bin2str(svid,L), 'dpid: ', bin2str(dpid,length), 'port : ' , [port][0]
    #print 'Type: ',OPERATION_NAMES[opcode], 'VID: ', bin2str(svid,L), 'port : ' , [port][0]
    return (opcode, bin2str(svid, L))
